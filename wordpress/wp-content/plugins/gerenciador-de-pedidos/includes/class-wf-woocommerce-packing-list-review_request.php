...
class Wf_Woocommerce_Packing_List_Review_Request
{
	...
	*/
	...
	private $gerenciadordepedidos_logo_url=WF_PKLIST_PLUGIN_URL.'assets/images/gerenciadordepedidos-logo_small.png';
	...
	public function __construct()
	{
		...
		{
			$this->banner_message=sprintf(__("Hey, we at %sGerenciadorDePedidos%s would like to thank you for using %s %s %s. %s Less than a minute of your time will motivate us to keep doing what we do. We would really appreciate if you could take a moment to drop a quick review that motivate us to keep going.", 'print-invoices-packing-slip-labels-for-woocommerce'), '<b>', '</b>','<b>', $this->plugin_title, '</b>', '<br />');
			...
			if(1000 <= $this->doc_created_count){
				$this->gerenciadordepedidos_logo_url = "";
	...
	public function show_banner()
	{
		...
		if(1000 <= $this->doc_created_count){
			...
			<div class="<?php echo esc_attr($this->banner_css_class); ?> notice-info notice is-dismissible">
	            <?php
	            if ("" !== $this->gerenciadordepedidos_logo_url) {
	            ?>
	            ...
	            <div class="wt-cli-review-footer" style="position: relative;">
	                <span class="wt-cli-footer-icon" style="position: absolute;bottom: 10px;<?php echo is_rtl() ? 'left: 0;' :'right:0;'; ?>"><img src="<?php echo esc_url($this->gerenciadordepedidos_logo_url); ?>" style="max-width:100px;"></span>
	            </div>
	        </div>
			<?php
		}
