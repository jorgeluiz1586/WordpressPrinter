 				...
 				if(wf_tab_elm.find(".wf-plugin-toolbar").length != 0) {
				    ...
				    if(jQuery(document).scrollTop() + window.innerHeight < wf_tab_elm.find('.end_wf_setting_form').offset().top){
				    	if(wf_tab_elm.parents().find('.wf_general_settings_form').length !== 0){
				    		wf_tab_elm.find(".wf-plugin-toolbar.bottom").css({'right':'25%'});
				    	}else{
				    		wf_tab_elm.find(".wf-plugin-toolbar.bottom").css({'right':'2%'});
				    	}
				        wf_tab_elm.find(".wf-plugin-toolbar.bottom").css({'bottom':'0','position':'fixed','background':'transparent','border':'none'});
...
var wt_pklist_free_vs_pro = {
	Set:function(){
		var wt_nav_tab=jQuery('.wf-tab-head .nav-tab');
	 	if(wt_nav_tab.length>0)
	 	{
	 		wt_nav_tab.on('click',function(){
	 			var wt_tab_hash=jQuery(this).attr('href'); 			
	 			wt_pklist_free_vs_pro.SidebarChange(wt_tab_hash);
	 		});

	 		if(jQuery('.wf-tab-head .nav-tab.nav-tab-active').length>0)
	 		{
	 			var current_hash=jQuery('.wf-tab-head .nav-tab.nav-tab-active').attr('href');
	 			wt_pklist_free_vs_pro.SidebarChange(current_hash);
	 		}
	 	}
	...
	SidebarChange:function(wt_tab_hash){
		wt_tab_hash = "#" === wt_tab_hash.charAt(0) ? wt_tab_hash.substring(1) : wt_tab_hash;
		if("freevspro" === wt_tab_hash)
		{
			jQuery('.wt_pklist_gopro_block, .wt_pklist_upgrade_to_pro_bottom_banner').show();
			jQuery('.wf_gopro_block').hide();
		}else
		{
			jQuery('.wt_pklist_gopro_block, .wt_pklist_upgrade_to_pro_bottom_banner').hide();
			jQuery('.wf_gopro_block').show();
		}
	}
}
